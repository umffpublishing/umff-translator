...
섹션 서문은 프로그래머·협력 기관이 해당 섹션 수록 작품 전체를 소개하는 큐레이터 글이다.
길이 상한이 없으며, 수록 작품명·감독명·주요 정보를 하나도 빠뜨리지 않는 것이 최우선이다.

#### 규칙 23 · 도입 문장 — 섹션명을 영문 큰따옴표로

본문 안에서 섹션명을 언급할 때 큰따옴표를 사용한다. 영문 섹션명은 `proper-nouns.csv`의 확정 표기를 그대로 쓴다.

```
KO: '자연' 섹션에서는 …
EN: The "Nature" section presents …

KO: '인간' 섹션은 "그 시절 우리는"과 신진 감독의 다채로운 시선으로 구성하였습니다.
EN: The "Human" section is composed of "Back in the Days" and works by emerging directors.
```

도입 동사는 문맥에 맞게 선택한다: `presents` (소개), `features` (내세움), `is composed of` (구성됨).

#### 규칙 24 · 작품 소개는 주제별 단락으로 묶는다

한국어의 긴 나열 문장을 영어에서 성격이 비슷한 작품끼리 묶어 단락을 구성한다.
한 단락 안의 여러 작품은 세미콜론이나 연결사(`while`, `also`, `in addition`)로 이어도 된다.

```
KO: <작품A>, <작품B>는 도전과 성취로 가득합니다.
EN: Film A and Film B are filled with stories of challenge and achievement.
```

#### 규칙 25 · 귀속 표시(attribution) 처리

`― 프로그래머 이름` 줄은 `― [Name], Programmer`로 번역한다.
협력 기관 귀속은 `― [Name], [Institution Name]`으로 처리한다.
영문명은 TM 또는 글로서리를 먼저 확인한다.

```
KO: ― 프로그래머 이정진
EN: ― Jinna Lee, Programmer

KO: ― 트렌토영화제 로잔나 스테딜레
EN: ― Rosanna Stedile, Trento Film Festival
```

#### 규칙 26 · 각주(`*`) 처리

`*`로 시작하는 협력 기관·협력 프로그램 소개문도 전문 번역한다.
기관명·영화제명은 `proper-nouns.csv` 또는 `festivals-schools.csv`의 확정 표기를 사용한다.

#### 규칙 27 · 길이 제한 없음

섹션 서문에는 조직위 공식 길이 상한이 없다.
원문의 모든 작품명·감독명·정보를 빠짐없이 번역하며, 내용 누락은 절대 허용하지 않는다.
한국어 긴 복문은 영어에서 2~3문장으로 나눠도 좋지만, 정보는 모두 유지한다.

---

### 2-5. 페스티벌 프로그램 (한→영)

자연에서 채우다·노래하다·이야기하다 등 부대 프로그램의 소개문·안내문·아티스트 소개를 번역하는 규칙이다.
길이 제한 없음. 원문 정보 전부 유지.

#### 규칙 28 · 청유·권유 문체를 영어에서도 유지한다

"해보세요", "함께해요", "초대합니다" 등 권유·청유형을 영어 명령형·청유형으로 자연스럽게 옮긴다.

```
KO: 영남알프스에서 활력을 되찾아 보세요.
EN: Recharge with activities everyone can enjoy in the Yeongnam Alps.

KO: UMFF가 어린이 여러분을 초대합니다.
EN: UMFF invites young audiences into a world of dreams and wonder!

KO: 함께해봅니다! / 지금 바로 참여해보세요.
EN: Join us! / Join now!
```

#### 규칙 29 · 불릿 구조(준비물·유의사항·프로그램)를 그대로 유지한다

항목 제목과 불릿 구조를 영어에서도 동일하게 유지한다.
...
불릿 기호(-, ⁃)는 원문 형식 그대로 유지한다.

#### 규칙 30 · 공연·행사 소개에서 아티스트 이름을 생략하지 않는다

바이오(규칙 9)와 달리, 공연·행사 소개에서는 아티스트·출연자 이름을 주어로 그대로 쓴다.

```
KO: 인순이는 세대를 초월하는 폭발적인 가창력과 무대 장악력으로...
EN: Insooni is a beloved diva whose powerful vocals and commanding stage presence...
```

#### 규칙 31 · 길이 제한 없음. 원문 정보 전부 유지

섹션 서문과 동일하게 길이 상한 없음. 내용 누락 불가.
